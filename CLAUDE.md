...
- **`src/postcard/core/`** — UI-agnostic logic, no GTK widgets. Sub-packages: `models/` (Account, Folder, Email, Conversation, Attachment as `GObject.Object`s so `Gio.ListStore` accepts them, plus `MessageHeader`, `MailboxInfo` and `PendingAction` as plain records), `store/database.py` (all SQLite), `net/` (`imap_session.py`, `smtp_session.py` — thin stdlib `imaplib`/`smtplib` wrappers; `graph_session.py`, `graph_folders.py`, `graph_messages.py`, `graph_send.py` — Microsoft Graph over `urllib`; `errors.py`), `mime/` (`message_parser.py`; `preview.py`, the list's preview line from a partial body; `invitation.py`, iCalendar invitations and iMIP replies), plus `threader.py`, `compose.py`, `secrets.py`. This is where testable logic belongs.
- **`src/postcard/*.py`** — the GTK layer. `application.py` (Adw.Application, app actions/accelerators), the main window (below), and per-view modules (`composer_window.py`, `message_view.py`, `conversation_row.py`, dialogs, `mail_sync.py`, `mail_watch.py`, `tray.py`).
...
Two more kinds of background thread follow the same rule. `mail_watch.MailWatcher` runs one per account while syncing is on: an IMAP account holds a second, unpooled connection in IDLE on the inbox (`ImapSession.wait_for_change`, spoken by hand since imaplib has no IDLE before 3.14, and stopped through a socketpair rather than a timeout), a Graph account polls the inbox counts every minute; either only schedules `_on_mail_arrived`, which starts the ordinary sync. And a flag change or move that finds no network is queued in `pending_actions` instead of reverted (`errors.is_connectivity` decides), then replayed by `mail_sync.replay` before the next sync of that account -- replay first, or the sync pulls the server's old flags over the user's.

### The smart inbox and the sidebar

Both are Spark's organization in libadwaita's look.

- **Categories** come from `core/categories.py`: header rules over `SIGNAL_HEADERS` (list and bulk-mail headers, `Auto-Submitted`, machine sender addresses, a calendar part), overridden on Gmail by its own tabs (`mail_sync._gmail_categories`, an `X-GM-RAW` search over the fetched UIDs), and above both by the user's per-sender rule in `sender_categories`, which `save_incoming_email` applies on insert. No model, no network: add a signal by adding its header to `SIGNAL_HEADERS`, which the IMAP fetch reads from.
- **Priority is the server's flag; pins are local.** Priority *is* the star -- IMAP `\\Flagged` / Graph's flag, synced and shown by every other client -- presented with the priority look (tinted row, `mail-mark-important-symbolic`, `win.toggle-star`, the sidebar's Priority group). The old local `emails.is_priority` column is unused; its marks were queued onto the server flag by a migration. Pins (`emails.is_pinned`, `win.toggle-pin`) stay on this machine and move a thread into the Pinned section on top. Neither a priority nor a pinned thread ever folds into a bundle.
- **Layout** is `core/smart_inbox.py`, pure and tested: `build()` turns the conversations of an inbox view into `InboxSection`s, `Bundle`s and conversations for the chosen view (`inbox-view` setting), and `open_bundle()` lists one bundle. The window's list store therefore holds three GObject types; read items with `isinstance`, and `inbox_rows.InboxItemRow` shows whichever kind a recycled row is bound to. Style an item through `InboxItemRow`'s own CSS classes (the list's rows have no padding, so it fills them); never reach the list's row widget with `get_parent()` from a bind handler -- that left a dangling reference that segfaulted on a later redraw. Bundles fold only in inbox views without a search, and a bundled account (`accounts.is_bundled`, set in the Accounts dialog) only in the cross-account Inbox.
...
- **A locally saved copy is adopted, not duplicated.** Sending files a copy under Sent, and saving a draft files it under Drafts (and uploads it), both with no server id. When a sync brings a message with the same Message-ID into that folder, `save_incoming_email` gives the local row the server id instead of inserting a second one, keeping its cached body. Moves made offline rely on the same match. So store the Message-ID on any row you save locally.
- Bcc never goes into a stored message; a queued one keeps its Bcc in `emails.bcc` so a retry from the Outbox still reaches them.
- IMAP syncs fetch `BODY.PEEK[TEXT]<0.2048>` beside the headers for the preview line, decoded by `core/mime/preview.py`; `imap_session.fetch_items` regroups imaplib's flat reply, since a message with two literals arrives as two tuples.
- Search runs locally at once and on the server of every folder on screen (`mail_sync.search_mailbox`); server hits are stored and passed to `search_conversations` as ids, since FTS never indexed the body they matched in.
