...
- **Priority** is `emails.is_priority`, set only by the user (`win.toggle-priority`) and never synced -- no server has a flag for it. A thread is priority if any of its messages is, so replies inherit it.
