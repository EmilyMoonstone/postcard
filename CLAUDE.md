...
- **Priority, pins and stars are three different things.** A star is IMAP `\Flagged` / Graph's flag: synced, and shown by every other client. Priority (`emails.is_priority`, `win.toggle-priority`) and pins (`emails.is_pinned`, `win.toggle-pin`) are set only by the user and stay on this machine -- no server has flags for them. Priority tints a thread where it sits; a pin moves it into the Pinned section on top. Either keeps it out of bundles. A thread is either if any of its messages is, so replies inherit it.
